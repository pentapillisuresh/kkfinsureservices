...
const createInvestment = async (req, res) => {
  ...
  try {
    const { userId, planId, amount, investmentDate, maturityDate, startReturnDate, endReturnDate } = req.body;
    ...
    const startReturnDateObj = new Date(startReturnDate);
    const endReturnDateObj = new Date(endReturnDate);

    // 5. Determine maturity date
    let finalMaturityDate;
    if (maturityDate) {
      finalMaturityDate = new Date(maturityDate);
    } else {
      // Calculate maturity date based on plan's maturity period
      finalMaturityDate = calculateMaturityDate(investmentDateObj, plan.maturityPeriod);
    }

    // 6. Create investment (InvestmentCode will be auto-generated by hook)
    const investment = await Investment.create({
      userId,
      planId,
      amount,
      investmentDate: investmentDateObj,
      startReturnDate: startReturnDateObj,
      endReturnDate: endReturnDateObj,
    ...
    // Determine first and last return months based on start/end return dates
    const firstReturnMonth = new Date(startReturnDateObj.getFullYear(), startReturnDateObj.getMonth() + 1, 1);
    const lastReturnMonth = new Date(endReturnDateObj.getFullYear(), endReturnDateObj.getMonth() + 1, 1);
      ...
      if (isSenior) {
        ...
        if (monthDiff % 3 === 0) {
          ...
          await Return.create({
            ...
            monthNo: monthNumber, // monthNumber is the current month count
